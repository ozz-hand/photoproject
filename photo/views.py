...
# django.views.generiecからDetailViewをインポート
...
# デコーレーターにより、CreatePhotoViewｈのアクセスはログインユーザーに限定される
# ログイン状態でければsetting.pyのLOGIN_URLにリダイレクトあれる
...
class CreatePhotoView(CreateView):
    ...
    def form_valid(self, form):
        ...
        フォームのバリデーションクラスを通過したときに呼ばれる
        フォームデータの登録をここで行う。
        
        parameters:
            form(django.forms.Form):
                form_classに格納されているPhotoPhotoFormオブジェクト
            Return:
...
class PhotoDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        Parameters:
            ...
            request: WSGURequest(HttpReuquest)オブジェクト
            args: 引数として渡される辞書(dict)
            kwargs: キーワード付きの辞書(dict)
            {'pk':21}のようにレコードのidが渡される
            
        Returns:
            HttpReqponseRedirect(success_url)を返して
            success_urlにリダイレクト
            '''
        # スーパークラスのdelete()を実行
